...
class Indikator extends CI_Controller
{
  ...
  {
    $results=$this->Indikator_model->get_all();
    print_r($this->get_menu($results,0));
  }
  ...
  public function get_menu($results,$parent_id)
  {
    ...
    {
      if($results[$i]->level==$parent_id)
      {
        if($this->has_child($results,$results[$i]->id_indikator))
        {
          $sub_menu= $this->get_menu($results,$results[$i]->id_indikator);
          $menu.='<li>'.$results[$i]->nama.'</li>';
          $menu.='<li>'.$sub_menu.'</li>';
        }
        else
        {
  ...
  public function level_dropdown($id_standar='',$selected='')
  {
    $pk='id_indikator';
    $ci = & get_instance();
		$cmb = "<select name='level' class='form-control'>";
    $ci->db->where('id_standar',$id_standar);
		$data = $ci->db->get('indikator')->result();
		foreach ($data as $row) {
...
'waktu' => date("Y-m-d h:i:s"),
'tabel' => 'indikator',
'id_aksi' => $this->input->post('id_indikator', TRUE),
);
$this->Log_model->insert($data_log);
